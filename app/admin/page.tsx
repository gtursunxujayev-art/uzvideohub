...
type User = { id: number; name: string | null; username: string | null; telegramId?: string; coins: number; isAdmin: boolean }
type Video = { id: number; title: string; description: string; url: string; isFree: boolean; price: number }

export default function AdminPage() {
  const [users, setUsers] = useState<User[]>([])
  const [videos, setVideos] = useState<Video[]>([])
  const [form, setForm] = useState({ title: '', description: '', url: '', price: 0, isFree: false })
  ...
  async function addVideo(e: React.FormEvent) {
    ...
    if (res.ok) {
      setForm({ title: '', description: '', url: '', price: 0, isFree: false })
      load()
    } else {
        ...
        <div style={{ display: 'grid', gap: 16, gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))' }}>
          {videos.map(v => (
            <div key={v.id} style={{ background: 'rgba(0,0,0,0.35)', padding: 12, borderRadius: 12 }}>
              <div style={{ background: 'rgba(255,255,255,0.06)', height: 140, borderRadius: 8, marginBottom: 8 }} />
